...
describe("DatabaseTransaction", function DatabaseTransactionSpecs() {
  ...
  describe("persistModels", () => {
    it("should call transactionDidCommitChanges with a change that contains the models", () => {
      runs(() => {
      ...
      waitsFor(() => {
        return this.database.transactionDidCommitChanges.callCount > 0;
      });
      runs(() => {
        const change = this.database.transactionDidCommitChanges.mostRecentCall.args[0];
  ...
  describe("unpersistModel", () => {
    ...
    it("should call transactionDidCommitChanges with a change that contains the model", () => {
      runs(() => {
        return this.transaction.execute(() => {
          return this.transaction.unpersistModel(testModelInstance);
        });
